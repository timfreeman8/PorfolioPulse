...
import { Settings, Bell, ChevronDown, Shield } from 'lucide-react'
...
export function TopBar() {
  ...
  const { activeMemberId, setActiveMember } = useViewStore()
  ...
  return (
    ...
    >
      {/* Brand — fixed width matches sidebar (224px) minus header px-4 (16px) = 208px,
          then nav starts aligned with page content (which has p-6 = 24px padding) */}
      <div className="flex items-center gap-3 shrink-0 w-[208px]">
        ...
        <span className="font-semibold text-sm text-white leading-tight">
          Store & Associate<br />Technology
        </span>
      </div>

      {/* Sub-nav — all items evenly spaced with consistent px-5 padding */}
      <nav className="flex items-center h-full pl-2">
        {NAV_ITEMS.map(link => (
          <button
            key={link}
          ...
          >
            {activeMember ? (
              <>
                ...
                <span className="text-sm font-medium">{activeMember.name.split(' ')[0]}</span>
              </>
            ) : (
              <>
